const futureFlowNotes = {
  "massage-education": [
    "How to Prepare for a Therapeutic Massage",
    "Massage Myths That Just Won't Die",
    "Massage for Headaches",
    "Can Massage Help Arthritis?",
    "Benefits of Regular Massage",
    "Choosing the Right Massage for Your Goals",
    "Why More Pressure Isn't Always Better",
    "Massage and the Nervous System",
    "How Massage Supports Recovery",
    "Can Massage Improve Mobility?",
    "How Stress Shows Up in the Body",
    "Understanding Trigger Points",
    "Therapeutic Massage vs. Spa Massage",
    "Why Recovery Isn't Lazy",
    "Massage for Runners",
    "Massage for Golfers",
    "Massage for Desk Workers",
    "Massage for Healthcare Workers",
    "Massage During Pregnancy",
    "Massage for Older Adults",
    "Massage and Sleep Recovery",
    "Massage and Anxiety",
    "Massage for Neck Pain",
    "Massage for Low Back Pain",
    "Massage and Flexibility",
    "Massage and Circulation",
    "Heat Before Massage?",
    "Ice After Massage",
    "How Long Do Massage Benefits Last?",
    "When Should You Skip a Massage?"
  ],

  "movement-mobility": [
    "Joint Health and Healthy Movement",
    "How Often Should You Stretch?",
    "Mobility After 40",
    "Standing Better Throughout the Day",
    "Can Massage Improve Mobility?",
    "Why Strength Supports Mobility",
    "How Your Feet Affect Your Entire Body",
    "Why Your Ankles Matter More Than You Think",
    "The Difference Between Tightness and Weakness",
    "Why Glute Strength Matters",
    "How Massage Fits Into a Long-Term Mobility Plan",
    "The Best Mobility Exercises for Office Workers",
    "Daily Habits That Keep You Moving Well",
    "Is Your Core Really Weak?",
    "Why Your Neck Gets Stiff",
    "Simple Ways to Improve Shoulder Mobility",
    "Mobility vs Stability: Why You Need Both",
    "Walking vs Stretching: Which Should You Do?",
    "Active Recovery vs Complete Rest",
    "Foam Rolling vs Massage",
    "Why Movement Snacks Work",
    "How Long Does It Take to Improve Mobility?",
    "What Happens When You Stop Moving?",
    "Why Your Body Feels Stiffer in the Morning",
    "How Breathing Changes the Way You Move",
    "Why Rotation Matters",
    "Is Cracking Your Joints Harmful?",
    "How Stress Changes the Way You Move",
    "Building a Sustainable Movement Routine",
    "Why Recovery Days Improve Performance",
    "When Mobility Exercises Make Symptoms Worse"
  ],

  recovery: [
    "Foam Rolling: Helpful or Harmful?",
    "Does Massage Remove Lactic Acid?",
    "Recovery After Yard Work",
    "Delayed Onset Muscle Soreness Explained",
    "Signs You're Overtraining", 
    "Why Recovery Is Part of Fitness",
    "How Long Does Muscle Recovery Take?",
    "Can Massage Flush Toxins?",
    "Why Am I More Sore Two Days Later?",
    "Should You Stretch When You're Sore?",
    "How Long Should Muscle Soreness Last?",
    "Is Heat Good for Tight Muscles?",
    "Should You Exercise While Sore?",
    "Is Foam Rolling Better Than Massage?",
    "What Causes Muscle Knots?",
    "Why Do My Muscles Feel Tight All the Time?",
    "Why Am I Always Sore After Exercise?",
    "Massage vs Rest: Do You Need Both?",
    "The Recovery Habits That Matter Most",
    "How Stress Slows Recovery",
    "Massage as Part of a Recovery Routine",
    "Why Recovery Gets Harder With Age",
    "Creating Your Personal Recovery Plan",
    "Why Consistency Beats Recovery Hacks",
    "Delayed Onset Muscle Soreness Explained",
    "Signs You're Overtraining",
    "Why Recovery Is Part of Fitness",
    "Why Rest Isn't Laziness",
    "Nutrition and Muscle Recovery",
    "Recovering After Moving Heavy Furniture",
    "Recovery After Gardening",
    "Recovery for Busy Parents",
    "Recovering After Long Workdays",
    "What Recovery Really Means",
    "Can You Recover Too Much?",
    "The Recovery Mistakes People Make Most",
    "Morning Recovery vs Evening Recovery",
    "Signs Your Body Needs More Recovery",
    "Recovery After Illness",
    "Recovering After Poor Sleep",
    "Why Gentle Movement Helps Recovery",
    "When Recovery Should Include Medical Care",
    "The Science Behind Recovery"
  ],

  "stress-wellness": [
    "Why Stress Affects Digestion", 
    "Why You Hold Your Breath Without Realizing It",
    "How to Recover From an Overwhelming Week",
    "Understanding the Fight, Flight, Freeze Response",
    "The Difference Between Rest, Recovery, and Relaxation",
    "Decision Fatigue and Physical Tension",
    "Why You Feel Tired Even After Sleeping",
    "Why Recovery Is Productive",
    "Why Your Shoulders Never Seem to Relax",
    "How Chronic Stress Affects Recovery",
    "The Health Benefits of Doing Nothing",
    "How Stress Changes Your Breathing",
    "Why Rest Isn't the Same as Laziness",
    "Building Healthy Recovery Habits",
    "Emotional Stress vs Physical Stress",
    "What Does Relax Actually Feel Like?",
    "Managing Stress During Busy Seasons",
    "When to Slow Down vs Push Through",
    "Understanding Mental Load and Physical Pain",
    "Small Daily Habits That Reduce Stress",
    "Creating a Sustainable Self-Care Routine"
  ],

  "workplace-wellness": [
    "Your Commute Counts as Physical Stress",
    "Why You Feel Fine at Work but Hurt at Home",
    "How Stress Changes Your Sitting Posture",
    "The Difference Between Good Posture and Comfortable Posture",
    "The Afternoon Slump Isn't Always About Sleep",
    "Microbreaks Beat Marathon Stretch Sessions",
    "When Your Desk Setup Isn't the Real Problem",
    "The Cost of Skipping Lunch (for Your Muscles Too)",
    "Why Your Body Doesn't Care That You're Almost Done",
    "Laptop Ergonomics",
    "Stretching at Your Desk",
    "Best Sitting Positions",
    "Office Wellness Habits",
    "Why Your Wrists Hurt at the Computer",
    "How to Sit Without Your Back Hurting",
    "Why Your Hips Get Tight at Work",
    "The Hidden Cost of Sitting All Day",
    "Simple Ways to Reduce Workplace Stress",
    "Why You Clench Your Jaw at Work",
    "Standing All Day Isn't the Answer Either",
    "Healthy Habits for Remote Workers",
    "Office Recovery After a Long Day",
    "What Your Work Bag Is Doing to Your Body",
    "The Best Way to Carry a Backpack",
    "Headaches That Start at Your Desk",
    "Why Your Feet Hurt After Work",
    "Workplace Burnout vs Physical Fatigue",
    "Healthy Computer Habits Everyone Should Know",
    "How Often Should You Get Up From Your Desk?",
    "Why Your Shoulders Stay Elevated",
    "The Ergonomic Mistakes Almost Everyone Makes",
    "The Best Mouse, Keyboard, and Monitor Position",
    "Creating a Workspace Your Body Likes",
    "Massage for Office Workers",
    "Building a Sustainable Workday Routine"
  ]
};

const futureSection = document.querySelector("[data-future-category]");

if (futureSection) {
  const category = futureSection.getAttribute("data-future-category");
  const list = futureSection.querySelector(".future-flownotes-list");
  const topics = futureFlowNotes[category];

  if (list && topics) {
    list.innerHTML = "";

    topics.slice(0, 3).forEach(function (topic) {
      const item = document.createElement("li");
      item.textContent = topic;
      list.appendChild(item);
    });
  }
}